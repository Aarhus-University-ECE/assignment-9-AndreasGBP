...
void DFT (node * root)
{
  ...
  //This function pushes the root node
  printf("%d, ", root->num);
  ...
  while (!done) 
  {
    while(temp -> lchild !=NULL) //Keeps moving thorugh the elements to the left as far as possible, and notes them as marked and "visited"
    {
      temp = temp -> lchild;
      printf("%d, ,", temp -> num); 
  ...
  // Checks if the nodes has any child nodes left to examine. If not its finds own parent node, and temporarily points to it. 
...
node *make_node (int num, node * left, node * right)
{
  ...
  //Node with num, left & right child and wether or not it has been "visited"
  (treePointer) -> num = num;
...
stack *push (stack * topp, node * node)
{
  ...
  t -> node = node; //Add previous topp node
  return t;   
}
